...
(function($) {
    ...
    Window.prototype.initialize = function(options) {
        ...
        if (options.elements.zIndex)
            this.$el.css('zIndex', options.elements.zIndex);
        else
            this.$el.css('zIndex', 900);
        ...
        if (_this.$el.find('[data-dismiss=window]').length <= 0) {
            options.elements.handle.prepend('<button type="button" class="close" data-dismiss="window" aria-hidden="false"><i class="glyphicon glyphicon-remove"></i></button>');
        }
    ...
    Window.prototype.centerWindow = function() {
        var top, left, bodyTop = parseInt(this.options.references.body.position().top, 10) + parseInt(this.options.references.body.css('paddingTop'), 10), maxHeight;
